//utf-8 width module ccchart.m.cssHybrid http://jsgt.org/c/
...
(function(window) {
    return {
            aboutThis: {
                version: '0.954-b1',
                update: 20121110,
                updateMemo: 'http://jsgt.org/c/update.json',
            ...
            init: function (id, op, callback) {
                ...
                if(this.adds){
                  ...
                  if(hybrid){
                    if(this.ws){
                      if(this.ws[this.id]){
                        if(this.ws[this.id].readyState=== this.ws[this.id].OPEN){
                          //don't remove
                        }
            ...
            ws: function(url, port){
              var _ = function(){
                var _ws = (function (){
                  return new (window.WebSocket || window.MozWebSocket)
                    ('ws://' + url + ':'+ port +'/');
                })(url, port);
                _ws.onopen = function (fn){ if(typeof fn==='function')fn.apply(this,[_ws]) }
                _ws.onmessage = function (data, fn){ if(typeof fn==='function')fn.apply(this,[_ws]) }
                _ws.onclose = function (fn) { if(typeof fn==='function')fn.apply(this,[_ws]) }
                return _ws
              }
              return new _();
            },
            ...
            drawXscale: function (left, count){
                ...
                if(this.type==='scatter'){
                  ...
                  val = this.util.addComma(this.wkXScale);
            ...
            drawYscale: function(top, count){
                ...
                if(this.type === 'stacked%'){
                    ...
                    val = this.util.addComma(this.wkYScale);
            ...
            _drawLine: function (op, func) {
                ...
                this.ctx.restore();
                this.drawing = false; if(this.ondrew)this.ondrew(this);
            ...
            _drawStackedArea: function (op, func) {
                ...
                var lineWidth = op.lineWidth || this.lineWidth;
                var colorSet = op.colorSet || this.colorSet;
                var shdw = this.shadows.stackedarea;
                var wkData = [];
                var wkMaxYs = [];
                for(var k = 0; k < this.dataRowLen; k++ ){
                    wkData[k] = [];
            ...
            _moveByWS: function (cvs, chartdata, url, op, delay, step) {
              ...
              if(!this.ws) {
                this.ws = [];
                this.ws[id] = mkWS(); // ws無ければ接続
              }
              else if(!this.ws[id]) {
                this.ws[id] = mkWS();
              }
              else {
                if(this.ws[id].readyState !== this.ws[id].OPEN)
                  this.ws[id] = mkWS(); // 切れたら再接続
              }
              this.ws[id].cchart = this;
              this.ws[id].onopen = function () { console.log('ws opened: ' + url); }
              this.ws[id].onclose = function () { console.log('ws closed: ' + url); }
              this.ws[id].onmessage = function (e) {
                ...
                try {
                  ...
                  if(movebywsInitFlg === 0) {//初回は即実行
                    var dataid = getDataId(wsq,  op, cvs);
                    drawIt(e, id, config, dataid[0], cvs, chartdata, dataid[1], wsq);
                  }

                  //描画速度調整用タイマー開始
                  // (サーバープッシュ過剰時のクライアント負荷も回避する)
                  clearTimeout(wstid[newid]);
                  timerDrawing[newid] = true;              // タイマ描画開始
              ...
              function getDataId(wsq,  op, cvs){
                //wsq = wsq.slice(0, 1);  // (a)に対応 先頭の最新データのみ受け取る。残りは捨てる
                   //(a)ではサーバーからのDosに負ける
                return mkdata(wsq, op, id); // type別にデータ追加と削除
              }

              function drawIt(e, id, config, newData, cvs, chartdata, newid, wsq){
