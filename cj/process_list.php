...
<?php if (($auth->getParent_role_process() & ROLE_MEMBER_VIW) == 0) { header("HTTP/1.0 404 Not Found"); exit(); } ?>
<?php require_once(dirname(__FILE__).'/../Encode.php'); ?>
<?php require_once(dirname(__FILE__).'/../db_config.php'); ?>
<?php require_once(dirname(__FILE__).'/class/ProcessListSQL.class.php'); ?>
...
<head>
<meta charset="utf-8">
<title><?php echo APP_TITLE; ?> | 工程マスタ</title>
...
<link rel="stylesheet" href="<?php echo PROJECT_ROOT ?>/cj/css/process_list.css" media="all">
...
<?php
	$process = new ProcessListSQL($dbopts);
...
<?php if (isset($_SESSION[S_MESSAGE2]) && strlen($_SESSION[S_MESSAGE2])) {
	...
	<div class="item_list">
		...
		<form id="post_form" method="post" action="process_list_control.php" >
			...
			<?php if ($list) { foreach ($list as $row) { ?>
				<tr class="tr_item">
					...
					<td class="td_ope" nowrap>
						...
						<?php if (($auth->getParent_role_process() & ROLE_MEMBER_EDT) != 0) { ?>
						...
						<?php } ?>
						<?php if (($auth->getParent_role_process() & ROLE_MEMBER_ADD) != 0) { ?>
							<input type="button" value="追加" class="btn_add" onclick="insertRow(this)"/>
						<?php } ?>
						<?php if (($auth->getParent_role_process() & ROLE_MEMBER_DEL) != 0) { ?>
...
function insertRow(obj) {
	...
	$('#validateTips').html('追加する工程名を入力してください。');
	$('#validateTips').css('color','blue');
	$('#ui-dialog-title-edit_dialog').html('工程追加');
	$('#label-editstr').html('工程名');
...
function updateRow(obj) {
	...
	$('#editstr').css('display','inline');
	$('#editstr-disabled').css('display','none');
	$('#validateTips').html('工程名を編集してください。');
	$('#validateTips').css('color','blue');
	$('#ui-dialog-title-edit_dialog').html('工程名の変更');
	$('#label-editstr').html('工程名');
	$('#edit_dialog').dialog('open');
}
...
function deleteRow(obj) {
	...
	$('#validateTips').html('この工程を削除しますか？');
	$('#validateTips').css('color','red');
	$('#ui-dialog-title-edit_dialog').html('工程削除');
	$('#label-editstr').html('工程名');
	$('#edit_dialog').dialog('open');
}
