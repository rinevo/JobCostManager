...
class CustomerListSQL {

	protected $opts = array('dsn'=>'', 'db_user'=>'', 'db_pwd'=>0);
	protected $usertable = T_CUSTOMER;
	protected $message = '';
	...
	function getList() {
		...
		try {
			...
			$sql.= ',(SELECT COUNT(no) FROM '.T_SCHEDULE.' WHERE customer_no=a.no) AS schedule_use ';
			$sql.= ',(SELECT COUNT(no) FROM '.T_TODO.' WHERE customer_no=a.no) AS todo_use ';
	...
	function insertItem($name, $sortno = '', $select_group_no = '') {
		...
		try {
			...
			$this->message = '顧客の追加に失敗しました。<br>'.$e->getMessage();
			$db->rollBack();
		}
		$db = NULL;
	...
	function updateItem($no, $name) {
		...
		try {
			...
			$this->message = '顧客名の変更に失敗しました。<br>'.$e->getMessage();
			$db->rollBack();
		}
		$db = NULL;
	...
	function deleteItem($no) {
		...
		try {
			...
			$sql = 'SELECT ';
			$sql.= '(SELECT COUNT(no) FROM '.T_SCHEDULE.' WHERE customer_no=:no1) AS schdule_use ';
			$sql.= ',(SELECT COUNT(no) FROM '.T_TODO.' WHERE customer_no=:no2) AS todo_use ';
			$stmt = $db->prepare($sql);
			...
			$this->message = '顧客の削除に失敗しました。<br>'.$e->getMessage();
			$db->rollBack();
		} catch(Exception $e) {
	...
	function moveItem($no, $before_sortno, $after_sortno) {
		...
		try {
			...
			$this->message = '顧客の並び替えに失敗しました。<br>'.$e->getMessage();
			$db->rollBack();
		} catch(Exception $e) {
			$this->message = $e->getMessage();
			$db->rollBack();
